...
from NetworkLit import GOModuleLit
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train GO Module')
...
if __name__ == '__main__':
    ...
    logger = TensorBoardLogger("lightning_logs", name="GO/" + args.dataset_path.split("/")[-1])
    checkpoint_callback = pl.callbacks.ModelCheckpoint(monitor="Val_MAE", mode='min', save_top_k=1, save_last=False)
    ...
    torch.save(model.model.state_dict(), "lightning_logs/GO_module_"+args.robot+".pt")
